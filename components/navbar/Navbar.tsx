 ...
 * Chức năng:
 * - Hiển thị logo và menu chính
 * - Menu responsive (desktop/mobile)
 * - Hiển thị trạng thái đăng nhập
 * - Quick access tới các chức năng chính (đăng tin, yêu thích, tin nhắn)
 */
...
const navigation = [
  ...
  { name: 'Đăng tin', href: '/listings/create', requireAuth: true }, // Yêu cầu đăng nhập
...
export default function Navbar({ currentUser }: NavbarProps) {
  ...
  // Xử lý điều hướng và kiểm tra auth
    ...
    <nav className="bg-white shadow-sm border-b">
      <div className="container px-4 mx-auto">
        <div className="flex items-center justify-between h-16">
          ...
          {/* User Menu - Hiển thị các action khi đã đăng nhập */}
          <div className="hidden md:block">
            <div className="flex items-center ml-4 space-x-4">
              {currentUser ? (
                ...
                // Hiển thị nút đăng nhập/đăng ký khi chưa auth
                <>
                  <Link
                    href="/login"
          ...
          <div className="px-2 pt-2 pb-3 space-y-1 bg-white sm:px-3">
            {navigation.map((item) => (
              <button
                ...
                className={`block w-full px-3 py-2 text-left text-base font-medium rounded-md ${
                  pathname === item.href
                    ? 'text-primary-600 bg-primary-50'
                    : 'text-gray-700 hover:text-primary-600 hover:bg-gray-50'
                }`}
